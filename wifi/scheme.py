...
class Scheme(object):
    ...
    def activate(self):
        ...
        ifup_output = subprocess.check_output(['/sbin/iwconfig', self.interface, 'essid', self.name, 'channel', 'auto'], stderr=subprocess.STDOUT)
        ifup_output = ifup_output.decode('utf-8')
        subprocess.check_output(['/sbin/ifconfig', self.interface, 'up'], stderr=subprocess.STDOUT)
...
scheme_re = re.compile(r'iface\s+(?P<interface>[^\s]+)(?:\s(?P<name>\S+))?')
